...
if supercomp == "satori":
    # Raw data:
    dir_data_raw = "/nobackup/users/vianajr/cluster_ages/data/raw_data/raw_phangs_dataset.h5"
     # Results parent directory:
    dir_results_parent = "/nobackup/users/vianajr/cluster_ages/results/single_case/"
else:
    # Raw data:
...
results_prefix = f"single_case_5im_autoencoder_remextremes_{txt_extremes}_augment_{txt_augment}_"
...
def plot_loss_over_epochs(loss, val_loss):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(dir_results_model + f"5_losses_evolution.png")
    plt.show()
...
def plot_pred_vs_true(y_true, y_pred, title, which_set):
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig(dir_results_model + f"6_pred_vs_true_{which_set}.png")
    plt.show()
...
def create_model(input_shape=(112, 112, 5), latent_dim=4):
    input_img = layers.Input(shape=input_shape)

    # ----- ENCODER -----
    x1 = layers.Conv2D(64, (3, 3), activation='relu', padding='same')(input_img)
    p1 = layers.MaxPooling2D((2, 2), padding='same')(x1)  # -> 56x56

    x2 = layers.Conv2D(128, (3, 3), activation='relu', padding='same')(p1)
    p2 = layers.MaxPooling2D((2, 2), padding='same')(x2)  # -> 28x28

    x3 = layers.Conv2D(256, (3, 3), activation='relu', padding='same')(p2)
    p3 = layers.MaxPooling2D((2, 2), padding='same')(x3)  # -> 14x14

    # ----- LATENT VECTOR -----
    xflat = layers.Flatten()(p3)
    latent = layers.Dense(
        latent_dim,
        activation='sigmoid',
        name='latent_vector',
    ...
    # ----- DECODER -----
    x = layers.Dense(14 * 14 * 64, activation='relu')(latent)
    x = layers.Reshape((14, 14, 64))(x)

    x = layers.Conv2DTranspose(256, (3, 3), strides=1, activation='relu', padding='same')(x)

    x = layers.Conv2DTranspose(128, (3, 3), strides=2, activation='relu', padding='same')(x)  # -> 28x28

    x = layers.Conv2DTranspose(64, (3, 3), strides=2, activation='relu', padding='same')(x)  # -> 56x56

    x = layers.Conv2DTranspose(32, (3, 3), strides=2, activation='relu', padding='same')(x)  # -> 112x112
    decoded = layers.Conv2D(input_shape[-1], (3, 3), activation='sigmoid', padding='same')(x)

    # ----- COMPILE MODEL -----
    autoencoder = models.Model(inputs=input_img, outputs=decoded, name='autoencoder_with_latent')
    return autoencoder
...
dir_results_model = dir_results + f"model/"
if not os.path.exists(dir_results_model): os.makedirs(dir_results_model)
...
for epochs, lr in zip(arr_epochs, arr_learn_rates):
    ...
    history = model.fit(
        nrm_X_tr, nrm_X_tr, 
        validation_data=(nrm_X_vl, nrm_X_vl),
        epochs=epochs, 
...
with open(dir_results_model + 'loss_data.pkl', 'wb') as f:
...
test_loss = model.evaluate(nrm_X_ts, nrm_X_ts, verbose=0)
...
def plot_input_and_reconstruction(X_data, autoencoder, dataset_label=""):
    """
    Randomly selects one instance from the dataset and plots its 5-channel input and reconstruction.
    
    Parameters:
    - X_data: Input data (N, 112, 112, 5)
    - autoencoder: Trained Keras model
    - dataset_label: Optional label for the plot (e.g., 'Train', 'Validation', 'Test')
    """
    idx = np.random.randint(0, len(X_data))
    x = X_data[idx:idx+1]  # Keep batch dimension for prediction

    recon = autoencoder.predict(x, verbose=0)[0]  # (112, 112, 5)
    ...
    for i in range(5):
        ...
        axs[0, i].axis('off')
        axs[0, i].set_title(f"Input - Ch {i+1}")

        axs[1, i].imshow(recon[:, :, i], cmap='gray')
        axs[1, i].axis('off')
        axs[1, i].set_title(f"Reconst. - Ch {i+1}")

    plt.suptitle(f"Input vs Reconstruction - {dataset_label} Set (Sample #{idx})", fontsize=14)
    plt.tight_layout()
    plt.savefig(dir_results_model + f"reconstructions_{dataset_label}.png")
    plt.show()

    
    
# Plot:
plot_input_and_reconstruction(nrm_X_tr, model, dataset_label="Train")
plot_input_and_reconstruction(nrm_X_vl, model, dataset_label="Validation")
plot_input_and_reconstruction(nrm_X_ts, model, dataset_label="Test")



from tensorflow.keras.models import Model

# STEP 1: Extract encoder model from the trained autoencoder
def extract_encoder(autoencoder, latent_layer_name='latent_vector'):
    encoder_output = autoencoder.get_layer(latent_layer_name).output
    encoder = Model(inputs=autoencoder.input, outputs=encoder_output)
    return encoder

# Assuming you have a trained autoencoder already
encoder = extract_encoder(model)  # `model` here is your trained autoencoder

# STEP 2: Predict the latent space for each set
latent_tr = encoder.predict(nrm_X_tr, verbose=0)
latent_vl = encoder.predict(nrm_X_vl, verbose=0)
latent_ts = encoder.predict(nrm_X_ts, verbose=0)

# STEP 3 & 4: Plot the first two latent features
def plot_latent_spaces(latent_tr, latent_vl, latent_ts):
    fig, axs = plt.subplots(1, 3, figsize=(15, 5))
    cmap = 'jet'
    
    # Train
    sc0 = axs[0].scatter(latent_tr[:, 0], latent_tr[:, 1], c=nrm_Y_tr, cmap=cmap, alpha=0.6, s=10)
    axs[0].set_title("Train Set Latent Space")
    axs[0].set_xlabel("Latent Dim 1")
    axs[0].set_ylabel("Latent Dim 2")

    # Validation
    sc1 = axs[1].scatter(latent_vl[:, 0], latent_vl[:, 1], c=nrm_Y_vl, cmap=cmap, alpha=0.6, s=10)
    axs[1].set_title("Validation Set Latent Space")
    axs[1].set_xlabel("Latent Dim 1")
    axs[1].set_ylabel("Latent Dim 2")

    # Test
    sc2 = axs[2].scatter(latent_ts[:, 0], latent_ts[:, 1], c=nrm_Y_ts, cmap=cmap, alpha=0.6, s=10)
    axs[2].set_title("Test Set Latent Space")
    axs[2].set_xlabel("Latent Dim 1")
    axs[2].set_ylabel("Latent Dim 2")
    
    # Add colorbar at far right
    cbar_ax = fig.add_axes([1.02, 0.15, 0.015, 0.7])  # [left, bottom, width, height]
    cbar = fig.colorbar(sc2, cax=cbar_ax)
    cbar.set_label("Normalized Log Age")


    plt.tight_layout()
    plt.savefig(dir_results_model + f"latentes_scatterplot.png")
    plt.show()

# Call the plotting function
plot_latent_spaces(latent_tr, latent_vl, latent_ts)


# Save the computed metrics in a pickle file
with open(dir_results_model + 'final_metrics.pkl', 'wb') as f:
    pickle.dump({
        'tr_final_MSE_nrm': loss[-1],
        'vl_final_MSE_nrm': val_loss[-1],
        'ts_final_MSE_nrm': test_loss}, f)
