...
flag_use_augmented = True

# If we are blacking out:
flag_black = False

# Choose the normalization method:
norm_by = "single-image"  # "dataset", "single-image"   # dataset can't do since perturbs. need to tackle separately.
...
results_prefix = f"single_case_1im_extremes_{txt_extremes}_augment_{txt_augment}_"
...
for k in range(num_models_per_case):
    ...
    # Extract loss, validation loss, and custom metric history from the history object
