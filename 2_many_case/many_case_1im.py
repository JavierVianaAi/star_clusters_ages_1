...
# Define the radius for the blacking of the center, if we do top_R = 0 then we just run 1 case, with no blacking:
top_R = 40
...
dir_results_parent = "/orcd/pool/005/vianajr/cluster_ages_1/results/many_case/"
...
results_prefix = f"many_case_1im_remextremes_{txt_remextremes}_augment_{txt_augment}_"
...
dir_top_results = dir_results_parent + results_prefix + f"blackout_{extension}_normby_{norm_by}/"
...
for R in arr_R:
    ...
    if R != 0:
        ...
        # Get the black set:
        blck_X = blackout(cur_X.copy(), R, flag_black_inner)
    ...
    for k in range(num_models_per_case):
        ...
        nrm_Y_tr_pred = np.ravel( model.predict(nrm_X_tr) )
        nrm_Y_vl_pred = np.ravel( model.predict(nrm_X_vl) )
        nrm_Y_ts_pred = np.ravel( model.predict(nrm_X_ts) )
        ...
        test_loss = model.evaluate(nrm_X_ts, nrm_Y_ts)
        ...
        # Group the predictions and the truths for storage ------------------------------------
        
        # Create DataFrames for each set
        tr_df = pd.DataFrame({
            "y_pred": dnrm_Y_tr_pred,
            "y_true": use_Y_tr,
            "galaxy_id": use_tr_galax_ids,
            "cluster_id": use_tr_clust_ids
        })
        
        vl_df = pd.DataFrame({
            "y_pred": dnrm_Y_vl_pred,
            "y_true": use_Y_vl,
            "galaxy_id": use_vl_galax_ids,
            "cluster_id": use_vl_clust_ids
        })
        
        ts_df = pd.DataFrame({
            "y_pred": dnrm_Y_ts_pred,
            "y_true": use_Y_ts,
            "galaxy_id": use_ts_galax_ids,
            "cluster_id": use_ts_clust_ids
        })
        
        # Combine into a dictionary
        results_dict = {
            "tr": tr_df.to_dict(orient="records"),
            "vl": vl_df.to_dict(orient="records"),
            "ts": ts_df.to_dict(orient="records")
        }
        
        # Save as JSON
        with open(os.path.join(dir_results_model_k, "predictions_and_errors.json"), "w") as f:
    ...
    # Averages:
    arr_avgs_tr_final_MSE_nrm.append( np.mean(all_tr_final_MSE_nrm) )
    arr_avgs_vl_final_MSE_nrm.append( np.mean(all_vl_final_MSE_nrm) )
    arr_avgs_ts_final_MSE_nrm.append( np.mean(all_ts_final_MSE_nrm) )
    arr_avgs_tr_final_MdAEs_dnrm.append( np.mean(all_tr_final_MdAEs_dnrm) )
    arr_avgs_vl_final_MdAEs_dnrm.append( np.mean(all_vl_final_MdAEs_dnrm) )
    arr_avgs_ts_final_MdAEs_dnrm.append( np.mean(all_ts_final_MdAEs_dnrm) )

    # Stdvs:
    arr_stdv_tr_final_MSE_nrm.append( np.std(all_tr_final_MSE_nrm) )
    arr_stdv_vl_final_MSE_nrm.append( np.std(all_vl_final_MSE_nrm) )
    arr_stdv_ts_final_MSE_nrm.append( np.std(all_ts_final_MSE_nrm) )
    arr_stdv_tr_final_MdAEs_dnrm.append( np.std(all_tr_final_MdAEs_dnrm) )
    arr_stdv_vl_final_MdAEs_dnrm.append( np.std(all_vl_final_MdAEs_dnrm) )
    arr_stdv_ts_final_MdAEs_dnrm.append( np.std(all_ts_final_MdAEs_dnrm) )
...
plt.errorbar(arr_R, arr_avgs_tr_final_MdAEs_dnrm, yerr=arr_stdv_tr_final_MdAEs_dnrm, 
             label='Tr. MdAE Dnrm.', capsize=3, fmt='-o')

plt.errorbar(arr_R, arr_avgs_vl_final_MdAEs_dnrm, yerr=arr_stdv_vl_final_MdAEs_dnrm, 
             label='Vl. MdAE Dnrm.', capsize=3, fmt='-o')

plt.errorbar(arr_R, arr_avgs_ts_final_MdAEs_dnrm, yerr=arr_stdv_ts_final_MdAEs_dnrm, 
             label='Ts. MdAE Dnrm.', capsize=3, fmt='-o')


plt.xlabel('R')
plt.ylabel('Median Absolute Error (Dnrm.)')
plt.title('Median Absolute Error  (Dnrm. units) vs. R')
plt.legend()
plt.grid(True)
plt.savefig(dir_top_results + f"7_MdAEs_dnrm_evolution.png")
plt.show()
...
plt.errorbar(arr_R, arr_avgs_tr_final_MSE_nrm, yerr=arr_stdv_tr_final_MSE_nrm, 
             label='Tr. MdAE Dnrm.', capsize=3, fmt='-o')

plt.errorbar(arr_R, arr_avgs_vl_final_MSE_nrm, yerr=arr_stdv_vl_final_MSE_nrm, 
             label='Vl. MdAE Dnrm.', capsize=3, fmt='-o')

plt.errorbar(arr_R, arr_avgs_ts_final_MSE_nrm, yerr=arr_stdv_ts_final_MSE_nrm, 
             label='Ts. MdAE Dnrm.', capsize=3, fmt='-o')

plt.xlabel('R')
plt.ylabel('MSE (Nrm.)')
plt.title('Mean Squared Error (Nrm. units) vs. R')
plt.legend()
plt.grid(True)
plt.savefig(dir_top_results + f"7_MSE_nrm_evolution.png")
plt.show()
